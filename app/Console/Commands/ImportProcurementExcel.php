...
/**
 * Bulk-import historical procurement records (Vendors, Committees, PR, PO, GR, PaymentMilestones)
 * from the Excel template generated by `procurement:generate-import-template`.
 *
 * Behaviour:
 *  - Wraps every write in a single DB transaction (rollback on any error)
 *  - Defaults to --dry-run; pass --force to actually persist
 *  - Records imported rows with an `Imported from Excel` note for traceability
 *  - Skips workflow events (DB writes only, no Event::dispatch)
 *  - Resolves all FKs (vendor_code → vendor.id, committee_code → user.id, pr_number → pr.id, …)
 *  - Validates enum values, dates, required columns
 *  - Reports errors per sheet/row before any commit
 */
class ImportProcurementExcel extends Command
{
    protected $signature = 'procurement:import-excel
                            {file : Path to the Excel file}
                            {--force : Actually persist changes (default is dry-run)}
                            {--company= : Default company_id when sheet rows omit it}';

    protected $description = 'Import historical PR / PO / GR / PaymentMilestone data from the Excel template';
    ...
    /** @var array<string, array{0:string,1:string}>  Errors grouped by sheet:row */
    private array $errors = [];

    /** @var array<string,int>  vendor_code => vendor.id */
    private array $vendorCodeMap = [];

    /** @var array<string,int>  committee_code => user.id (resolved via email) */
    private array $committeeUserMap = [];

    /** @var array<string,string>  committee_code => default_role */
    private array $committeeRoleMap = [];

    /** @var array<string,int>  pr_number => purchase_requisition.id */
    private array $prMap = [];

    /** @var array<string,int>  po_number => purchase_order.id */
    private array $poMap = [];

    /** @var array<string,int>  email (lowercased) => user.id */
    ...
    public function handle(): int
    {
        ...
        $this->info(($force ? '[REAL IMPORT]' : '[DRY-RUN]') . " Loading {$file} ...");
    ...
    private function importVendors($ss): void
    {
        ...
        // Pre-load existing vendors keyed by tax_id
        ...
        for ($r = 4; $r <= $highest['row']; $r++) {
            ...
            if (!in_array($status, self::VENDOR_STATUSES, true)) {
                $this->addError('Vendors', $r, "invalid status '{$status}'"); continue;
            }
            if (isset($this->vendorCodeMap[$code])) {
                $this->addError('Vendors', $r, "duplicate vendor_code '{$code}' in file"); continue;
            }

            // Tax ID handling: blank -> placeholder so we can still upsert uniquely
    ...
    // ==================================================================
    // Committees  (one row = a person/user with a default_role)
    // ==================================================================
    ...
    private function importPurchaseRequisitions($ss): void
    {
        ...
        for ($r = 4; $r <= $highest['row']; $r++) {
            ...
            if ($deptId === 0 || !Department::find($deptId)) {
                $this->addError('PR', $r, "department_id {$deptId} not found"); $bad = true;
            }
            if (!isset($this->userEmailMap[$reqEmail])) {
                $this->addError('PR', $r, "requester_email '{$reqEmail}' has no user"); $bad = true;
            }
            if (!in_array($method, self::PROCUREMENT_METHODS, true)) {
                $this->addError('PR', $r, "invalid procurement_method '{$method}'"); $bad = true;
            }
            if (!in_array($status, self::PR_STATUSES, true)) {
                $this->addError('PR', $r, "invalid status '{$status}'"); $bad = true;
            }
            if (!in_array($currency, self::CURRENCIES, true)) {
                $this->addError('PR', $r, "invalid currency '{$currency}'"); $bad = true;
            }
            if ($workType !== null && !in_array($workType, self::WORK_TYPES, true)) {
                $this->addError('PR', $r, "invalid work_type '{$workType}'"); $bad = true;
            }
            if ($formCat !== null && !in_array($formCat, self::FORM_CATEGORIES, true)) {
                $this->addError('PR', $r, "invalid form_category '{$formCat}'"); $bad = true;
            }
            ...
            if (isset($this->prMap[$prNumber])) {
                $this->addError('PR', $r, "duplicate pr_number '{$prNumber}' in file"); $bad = true;
            }

            $procComId = $inspComId = $apprUserId = null;
            if ($procCom !== '') {
                if (!isset($this->committeeUserMap[$procCom])) {
                    $this->addError('PR', $r, "procurement_committee_code '{$procCom}' not in Committees"); $bad = true;
                } else $procComId = $this->committeeUserMap[$procCom];
            }
            if ($inspCom !== '') {
                if (!isset($this->committeeUserMap[$inspCom])) {
                    $this->addError('PR', $r, "inspection_committee_code '{$inspCom}' not in Committees"); $bad = true;
                } else $inspComId = $this->committeeUserMap[$inspCom];
            }
            if ($apprEmail !== '') {
                if (!isset($this->userEmailMap[$apprEmail])) {
                    $this->addError('PR', $r, "pr_approver_email '{$apprEmail}' has no user"); $bad = true;
            ...
            // negotiated_price is not in fillable; skip unless column exists

            $pr->saveQuietly(); // bypass observers/events for historical records
            $this->prMap[$prNumber] = $pr->id;

            // Single PR item to capture budget breakdown
            try {
            ...
            // Committee linkage to PR is via *_id column already set; CommitteeMember is per-PO, not per-PR
    ...
    private function importPurchaseOrders($ss): void
    {
        ...
        for ($r = 4; $r <= $highest['row']; $r++) {
            ...
            if (!in_array($currency, self::CURRENCIES, true)) {
                $this->addError('PO', $r, "invalid currency '{$currency}'"); $bad = true;
            }
            if (!in_array($status, self::PO_STATUSES, true)) {
                $this->addError('PO', $r, "invalid status '{$status}'"); $bad = true;
            }
            if ($method !== null && !in_array($method, self::PROCUREMENT_METHODS, true)) {
                $this->addError('PO', $r, "invalid procurement_method '{$method}'"); $bad = true;
            }
            if ($workType !== null && !in_array($workType, self::WORK_TYPES, true)) {
                $this->addError('PO', $r, "invalid work_type '{$workType}'"); $bad = true;
            }
            if ($currency !== 'THB' && ($exchange === null || $exchange <= 0)) {
                $this->addError('PO', $r, "exchange_rate required when currency != THB"); $bad = true;
            }
            if (!isset($this->prMap[$prNumber])) {
                $this->addError('PO', $r, "pr_number '{$prNumber}' not found in PR sheet"); $bad = true;
            }
            if (!isset($this->vendorCodeMap[$vendorCode])) {
                $this->addError('PO', $r, "vendor_code '{$vendorCode}' not found in Vendors sheet"); $bad = true;
            }
            if (isset($this->poMap[$poNumber])) {
                $this->addError('PO', $r, "duplicate po_number '{$poNumber}' in file"); $bad = true;
            }

            $inspComId = null;
            if ($inspCom !== '') {
                if (!isset($this->committeeUserMap[$inspCom])) {
                    $this->addError('PO', $r, "inspection_committee_code '{$inspCom}' not in Committees"); $bad = true;
                } else $inspComId = $this->committeeUserMap[$inspCom];
            }
            if ($bad) continue;
            ...
            // CommitteeMember link (inspection committee → PO).
            // committee_members.role enum is ('chairman','member','secretary'),
            // so we map functional role -> chairman for the lead inspector
            // and member for the secondary procurement signer.
            if ($inspComId) {
    ...
    private function importGoodsReceipts($ss): void
    {
        ...
        for ($r = 4; $r <= $highest['row']; $r++) {
            ...
            if (!in_array($inspect, self::INSPECTION_STATUSES, true)) {
                $this->addError('GR', $r, "invalid inspection_status '{$inspect}'"); $bad = true;
            }
            if (!in_array($status, self::GR_STATUSES, true)) {
                $this->addError('GR', $r, "invalid status '{$status}'"); $bad = true;
            }
            if (!isset($this->poMap[$poNumber])) {
                $this->addError('GR', $r, "po_number '{$poNumber}' not found"); $bad = true;
            }
            if (isset($seen[$grNumber])) {
                $this->addError('GR', $r, "duplicate gr_number '{$grNumber}' in file"); $bad = true;
            }
            $rcvUserId = null;
            if ($rcvEmail !== '') {
                if (!isset($this->userEmailMap[$rcvEmail])) {
                    $this->addError('GR', $r, "received_by_email '{$rcvEmail}' has no user"); $bad = true;
    ...
    private function importPaymentMilestones($ss): void
    {
        ...
        for ($r = 4; $r <= $highest['row']; $r++) {
            ...
            if (!in_array($status, self::MILESTONE_STATUSES, true)) {
                $this->addError('PM', $r, "invalid status '{$status}'"); $bad = true;
            }
            if (!isset($this->poMap[$poNumber])) {
                $this->addError('PM', $r, "po_number '{$poNumber}' not found"); $bad = true;
            }
            if ($bad) continue;
            ...
            // upsert by (po_id, milestone_number)
    ...
    /**
     * Accept yyyy-mm-dd, dd/mm/yyyy, Excel serial. Return Y-m-d string or null.
     */
